...
export const ImageWithDescription = ({ image, description, textPosition = 'below' }: ImageWithDescriptionProps) => {
  ...
  return (
    <div className={`flex ${flexDirection} ${isHorizontal ? 'items-center lg:items-start' : 'items-center'} gap-2 h-full`} style={{ justifyContent: 'left' }}>
      {image && (
        <Image
          src={image}
      ...
      {description && description.length > 0 && (
        <div className={`prose prose-sm prose-gray dark:prose-invert max-w-none overflow-auto ${isHorizontal ? 'text-center lg:text-left lg:w-2/3' : 'text-center'}`}>
